...
function App() {
  ...
  const gameHtml = [
    ...
    '        if (done) break;',
    '        received += value.length; loadedBytes += value.length;',
    '        chunks.push(value);',
    '        loadingText.textContent = "LOADING... " + (loadedBytes / (1024 * 1024)).toFixed(2) + " MB / 132.83 MB";',
    '      }',
    '      let fullBuffer = new Uint8Array(received); let offset = 0;',
    ...
    '      if (cachedResponse) { const blob = await cachedResponse.blob(); return URL.createObjectURL(blob); }',
